...
def base_pos_z_beam(env: ManagerBasedEnv, slope_func: func, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """Relative root height on balance beam in the simulation world frame."""
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    beam_z = slope_func(asset.data.root_pos_w[:,0] - env.scene.env_origins[:,0])
    return (asset.data.root_pos_w[:,2] - beam_z).unsqueeze(-1)