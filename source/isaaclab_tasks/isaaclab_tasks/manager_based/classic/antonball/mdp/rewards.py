...
def pole_target_tracking(
    env: ManagerBasedRLEnv, object_name: str, target_pos: tuple[float, float, float],
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
...
def keep_orientation(
    env: ManagerBasedRLEnv, target_quat, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
