...
class HumandribbleEnv(LocomotionEnv):
    ...
    def __init__(self, cfg: HumandribbleEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.direction_quats = math_utils.quat_from_euler_xyz(torch.zeros_like(self.basis_vec0[:,0]), torch.zeros_like(self.basis_vec0[:,0]), 
    ...
    def _compute_intermediate_values(self):
        ...
        self.torso_rot_local = math_utils.quat_mul(math_utils.quat_conjugate(self.direction_quats), self.torso_rotation)
    ...
    def _change_direction(self, env_ids):
        ...
        self.direction_quats = math_utils.quat_from_euler_xyz(torch.zeros_like(self.basis_vec0[:,0]),
                                                              torch.zeros_like(self.basis_vec0[:,0]), 
                                                            self.direction_angles)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        left_foot_up = math_utils.quat_apply(left_foot_quat, self.basis_vec1)
        right_foot_up = math_utils.quat_apply(right_foot_quat, self.basis_vec1)
    ...
    def _get_observations(self) -> dict:
        ...
        torso_pos_local = math_utils.quat_apply_inverse(self.direction_quats, self.torso_position-self.last_pos_turn)
        ball_position = self.ball.data.root_pos_w
        ball_vel_lin = self.ball.data.root_lin_vel_w

        # calculate observation values
        ball_pos_b = math_utils.quat_apply_inverse(self.torso_rotation, ball_position-self.torso_position)
        ball_vel_b = math_utils.quat_apply_inverse(self.torso_rotation, ball_vel_lin)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        ball_vel_lin_local = math_utils.quat_apply_inverse(self.direction_quats, self.ball.data.root_lin_vel_w)
        ...
        ball_pos_local = math_utils.quat_apply_inverse(self.direction_quats, self.ball.data.root_pos_w-self.last_pos_turn)
        ...
        torso_heading_vec = math_utils.quat_apply(self.torso_rot_local, self.basis_vec0)
